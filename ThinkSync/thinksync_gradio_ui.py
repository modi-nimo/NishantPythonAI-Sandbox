...
def take_note_from_ui(note):
    retrieved_relevant_notes = think_sync_manager.retrieve_relevant_notes(note)
    think_sync_manager.take_notes(note)
    _action_items = think_sync_manager.extract_actions_from_note()
    return get_notes_ui(), retrieved_relevant_notes,_action_items,""
...
with demo:
    ...
    with gr.Tab("Relevant Notes"):
        relevant_notes = gr.Dataframe(label="Relevant Notes", type="pandas", interactive=False)
    ...
    note_ui.submit(take_note_from_ui, inputs=note_ui, outputs=[notes_list,relevant_notes,action_items,note_ui])
