document.addEventListener('DOMContentLoaded', function() {
  ...
  const commandSuggestions = document.querySelectorAll('.cmdSuggestion');
  ...
  if ('webkitSpeechRecognition' in window) {
    ...
    // Helper function to execute a command suggestion
    function executeCommandSuggestion(suggestion) {
      feedbackDiv.textContent = `Using suggestion: "${suggestion}"`;
      sendCommandToBackground(suggestion)
        .then(response => {
          if (response && response.success) {
            console.log('Ollama action received:', response.ollamaResponse);
            ...
            console.error('Error processing suggestion:', response ? response.error : 'No response');
            feedbackDiv.textContent = 'Error processing suggestion. ' + (response ? response.error : '');
            responseContainer.style.display = 'none';
          }
        })
        .catch(error => {
